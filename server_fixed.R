...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(SHP_REST_RAMSAR))   proxy %>% addPolygons(data = SHP_REST_RAMSAR,   color = "#00bcd4", weight = 1, fillOpacity = 0.15, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_AP))       proxy %>% addPolygons(data = SHP_REST_AP,       color = "#f39c12", weight = 1, fillOpacity = 0.15, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_FORESTAL)) proxy %>% addPolygons(data = SHP_REST_FORESTAL, color = "#2ecc71", weight = 1, fillOpacity = 0.2, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_RECARGA))  proxy %>% addPolygons(data = SHP_REST_RECARGA,  color = "#3498db", weight = 1, fillOpacity = 0.15, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_CAUCA))    proxy %>% addPolygons(data = SHP_REST_CAUCA,    color = "#2980b9", weight = 1, fillOpacity = 0.2, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_POBLADOS)) proxy %>% addPolygons(data = SHP_REST_POBLADOS, color = "#e67e22", weight = 1, fillOpacity = 0.2, group = "Zonas Restringidas")
    if (!is.null(SHP_REST_POZOS))    proxy %>% addPolygons(data = SHP_REST_POZOS,    color = "#9b59b6", weight = 1, fillOpacity = 0.2, group = "Zonas Restringidas")
  ...
  output$box_red <- renderValueBox({ 
    valueBox(sum(datos_r()$RIESGO=="CRITICO"), 
             "CRÍTICO", icon=icon("fire-alt"), color="danger", 
             subtitle = "Ventana de incendio inminente") 
  })
  output$box_orange <- renderValueBox({ 
    valueBox(sum(datos_r()$RIESGO=="ALTO"), 
             "ALTO", icon=icon("exclamation-triangle"), color="orange",
             subtitle = "Riesgo latente en el predio") 
  })
  output$box_yellow <- renderValueBox({ 
    valueBox(sum(datos_r()$RIESGO=="OBSERVACION"), 
             "OBSERVACIÓN", icon=icon("eye"), color="warning",
             subtitle = "Monitoreo preventivo") 
  })
  output$box_green <- renderValueBox({ 
    valueBox(sum(datos_r()$RIESGO %in% c("BAJO", "MITIGADO")), 
             "BAJO", icon=icon("check-circle"), color="success",
             subtitle = "Estabilidad ambiental") 
  })
